...
from PySide6.QtCore import QTimer, Qt, QDate, Signal
...
class MainWindow(QMainWindow):
    ...
    def _load_json(self):
        ...
        # Defer the dialog to the next event loop cycle so deleteLater() fully resolves first
        QTimer.singleShot(0, lambda: QMessageBox.information(self, "Loaded", f"Loaded {active} tasks."))
    ...
    def _refresh_empty(self):
        active = len([t for t in self.tasks if not t.get("done")]) > 0
        self.lbl_empty.setVisible(not active)
